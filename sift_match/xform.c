...
CvMat* lsq_homog( CvPoint2D64f* pts, CvPoint2D64f* mpts, int n )
{
	 ...
	 //只有旋转 和平移， 只需要解4个未知数， 只需要2个点
		CvMat* H, *A, *B, X;
		double x[5];//数组x中的元素就是变换矩阵H中的值
		int i;

		//将变换矩阵H展开到一个8维列向量X中，使得AX=B，这样只需一次解线性方程组即可求出X，然后再根据X恢复H
		/* set up matrices so we can unstack homography into X; AX = B */
		A = cvCreateMat(2 * n, 4, CV_64FC1);//创建2n*8的矩阵，一般是8*8
		B = cvCreateMat(2 * n, 1, CV_64FC1);//创建2n*1的矩阵，一般是8*1
		X = cvMat(4, 1, CV_64FC1, x);//创建8*1的矩阵，指定数据为x
		H = cvCreateMat(3, 3, CV_64FC1);//创建3*3的矩阵
		cvZero(A);//将A清零
		...
		//调用OpenCV函数，解线性方程组
		cvSolve(A, B, &X, CV_SVD);//求X，使得AX=B
		x[4] = 1.0;//变换矩阵的[3][3]位置的值为固定值1
		double xx[9] = {x[0],0,x[1],0,x[2],x[3],0,0,x[4]};
		CvMat XX = cvMat(3, 3, CV_64FC1, xx);
		cvConvert(&XX, H);//将数组转换为矩阵
		cvReleaseMat(&A);
		cvReleaseMat(&B);
		return H;
}
